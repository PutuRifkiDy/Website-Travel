...
@section('content')
    <section id="content" class="md:px-24 px-6 w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-8">
        <nav class="mt-8 px-4 w-full flex items-center justify-between">
            <a href="booking.html">
                <img src="{{ asset('assets/icons/back.png') }}" alt="back">
            </a>
            <p class="text-center m-auto font-semibold text-[24px] text-black/70">Checkout</p>
            <div class="w-12"></div>
        </nav>
        <div class="flex flex-col h-full justify-center px-4 gap-8 w-full">
            <div class="w-full h-auto shrink-0 flex justify-center items-center">
                <img src="{{ asset('assets/backgrounds/Bank-Account-Illustration.png') }}" class="object-contain md:max-w-[300px]"
                    alt="background">
            </div>
            <form action="{{ route('front.choose_bank_store', $packageBooking) }}" method="POST"
                class="flex flex-col gap-8 md:px-48 px-0">
                @csrf
                @method('PATCH')
